...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  func setupMenu() {
    ...
    let statusItem = NSMenuItem(
      title: "Status: \(getInstallStatus())", action: nil, keyEquivalent: "")
    statusItem.isEnabled = false
    menu.addItem(statusItem)

    // Version item
    let versionItem = NSMenuItem(
      title: "App Version: \(getAppVersion())", action: nil, keyEquivalent: "")
  ...
  func promptForInstallation() {
    ...
    alert.informativeText =
      "/vibe is not installed yet. Would you like to install it now?\n\nThis will open Terminal for an interactive installation."
    alert.alertStyle = .informational
  ...
  func openTerminalForInstall() {
    ...
    do {
      ...
      showAlert(
        title: "Error",
        message: "Failed to create installation script: \(error.localizedDescription)")
    }
  }
}
